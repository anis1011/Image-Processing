...
def calibrate_gauge(img):
    ...
    cv2.imwrite(workingFolder +'/images/logs/gray_masked_image.jpg', gray)
  

    #alpha = 1
    #dst = cv2.addWeighted(img2,alpha,img1,1-alpha,0)
    #dst = cv2.circle(img,(140,190), 40, (1,1,1), -1)
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1 ,300, param1=30, param2=70, minRadius=50, maxRadius=100)
    ...
    cv2.imwrite(workingFolder +'/images/logs/calibration.jpg', img)
    #get user input on min, max, values, and units
    #print 'gauge number: %s' %gauge_number
    
    """ min_angle = raw_input('Min angle (lowest possible angle of dial) - in degrees: ') 
    max_angle = raw_input('Max angle (highest possible angle) - in degrees: ') 
    min_value = raw_input('Min value: ') 
    max_value = raw_input('Max value: ') 
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r):
    ...
    cv2.imwrite(workingFolder +'/images/logs/threshhold.jpg', dst2)
    ...
    #    for x1, y1, x2, y2 in lines[i]:
     #       cv2.line(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
      #      cv2.imwrite(workingFolder +'/images/logs/gauge_all_lines.jpg', img)
   ...
   # testing only, show all lines after filtering
    #for i in range(0,len(final_line_list)):
        ...
        #cv2.line(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
        #cv2.imwrite('images/logs/gauge_all_lines_filtered.jpg', img)
    ...
    #print "Found lines after filtering reading: %s" %(len(final_line_list))
    ...
    cv2.imwrite(workingFolder +'/images/logs/line_overlay.jpg', img)
...
def main():
    #print "started"
    inputImages = []
    #print(os.getcwd() + "/images/input/*.jpg")
    # inputFolder = os.getcwd() + "/build/src/images/input/*.jpg"
    # workingFolder, filename = os.path.split(os.path.abspath(__file__))
    ...
    if (len(inputImages) > 0):
        for path in inputImages:
            try:
                ...
                # x, y are left corners where cropping starts
                #x = 120 
                #y = 80
                #h = 280
                #w = 350
                ...
                #draw circle from center of gauge image
                #cv2.circle(img, (width/2,height/2), 100, [0,0,0], thickness=5, lineType=8, shift=0)
                cv2.imwrite(workingFolder +'/images/logs/cropImg.jpg',img)        

                # Gauge circle calibration
                
                x, y, r = calibrate_gauge(img)
                
                # Gauge line calibration
                img2 = cv2.imread(workingFolder +'/images/logs/cropImg.jpg')
                val = get_current_value(img2, min_angle, max_angle, min_value, max_value, x, y, r)
        ...
        print("No images found for input")
        sys.stdout.flush()
